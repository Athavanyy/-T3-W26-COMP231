...
async function exportExecutiveRegistrations(eventId, eventTitle = "event") {
  const result = await api(`/executive/registrations/${eventId}/export`);
  const rows = result.data || [];

  if (!rows.length) {
    setMessage("No registrations found to export.", "error");
    return;
  }

  const csv = rowsToCsv(rows);
  const fileName = `event_registrations_${safeFileName(eventTitle)}_${eventId}.csv`;

  downloadTextFile(fileName, csv);
  setMessage("Event registrations exported.", "success");
}
...
async function loadExecutiveMembers(root) {
  ...
  document
    ...
    ?.addEventListener("submit", (e) => {
      e.preventDefault();
      loadMembershipHistory(formData(e.currentTarget));
    });
...
async function loadExecutiveEvents(root) {
      ...
      (event) => `
        <article class="item">
          ...
          <div class="actions">
            ...
            <button class="small secondary" data-export-regs="${event.event_id}">Export Registrations</button>
          </div>
        </article>
      `,
  ...
  root.querySelectorAll("[data-export-regs]").forEach((btn) =>
    btn.addEventListener("click", async () => {
      try {
        const event = events.find(
          (item) => String(item.event_id) === String(btn.dataset.exportRegs),
        );

        await exportExecutiveRegistrations(
          btn.dataset.exportRegs,
          event?.title || "event",
        );
      } catch (error) {
        setMessage(error.message, "error");
...
async function loadAdminAnnouncements(root, filters = {}) {
  ...
  root.innerHTML = `
    <section class="item">
      ...
      <form id="announcement-filter-form" class="form-grid three">
        <div class="field">
          <label>Keyword</label>
          <input name="keyword" value="${escapeHtml(filters.keyword || "")}" placeholder="Search title, message, club" />
        </div>
              ...
              <td>
                ${
                  String(a.status || "").toUpperCase() === "REMOVED"
                    ...
                    : `<button class="small danger" data-remove-announcement="${a.announcement_id}">Remove</button>`
                }
              </td>
            </tr>`,
            )
  ...
  document
    ...
    ?.addEventListener("submit", (e) => {
      e.preventDefault();
      loadAdminAnnouncements(root, formData(e.currentTarget));
    });

  root.querySelectorAll("[data-remove-announcement]").forEach((btn) =>
    btn.addEventListener("click", async () => {
      const ok = confirm("Remove this announcement from visible listings?");
      if (!ok) return;
      ...
      <section class="item">
        ...
        <form id="report-form" class="form-grid">
          ...
          <div class="form-actions full">
            ...
            <button type="button" class="secondary" id="export-report-button">Export Report CSV</button>
          </div>
        </form>
